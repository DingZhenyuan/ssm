...
public class ImageUtil {
	...
	public static String generateThumbnail(ImageHolder thumbnail,
			String targetAddr, String resourcesPath) {
		// 获取不重复的随机名
		String realFileName = getRandomFileName();
		// 获取文件的扩展名如png,jpg等
		String extension = getFileExtension(thumbnail.getImageName());
		// 如果目标路径不存在，则自动创建
		makeDirPath(targetAddr, resourcesPath);
		// 获取文件存储的相对路径(带文件名)
		String relativeAddr = targetAddr + realFileName + extension;
		// 获取文件要保存到的目标路径
		File dest = new File(resourcesPath + relativeAddr);
		...
		try {
			Thumbnails
					...
					.watermark(
							...
							ImageIO.read(new File(resourcesPath
									+ "/watermark.png")), 0.25f)
	 ...
	 * @param targetAddr
	 */
	private static void makeDirPath(String targetAddr, String resourcesPath) {
		String realFileParentPath = resourcesPath + targetAddr;
	 ...
	 * @param storePath
	 */
	public static void deleteFileOrPath(String storePath, String resourcesPath) {
		File fileOrPath = new File(resourcesPath + storePath);
